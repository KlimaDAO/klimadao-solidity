...
import type { ethers as TEthers, Signer } from "ethers";
...
task("fundedwallet", "Create a wallet (pk) link and fund it with deployer?")
  ...
  .setAction(async (taskArgs, { network, ethers }) => {
    ...
    try {
      localDeployerMnemonic = fs.readFileSync("./mnemonic.txt.txt").toString();
...
async function addr(ethers: typeof TEthers & HardhatEthersHelpers , addr: number | string) {
  if (typeof addr === typeof string && isAddress(addr as string)) {
    return getAddress((addr as string));
  }
  const accounts = await ethers.provider.listAccounts();
  if (accounts[addr as number] !== undefined) {
    return accounts[addr as number];
  }
...
function send(signer: Signer, txparams: Deferrable<TransactionRequest>) {
