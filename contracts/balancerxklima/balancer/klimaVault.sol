...
contract Vault is VaultAuthorization, FlashLoans, Swaps {
    ...
    address public klimaSwapRetirementHolder;
    uint256 public klimaRetirementFee;
    ...
    constructor(
        ...
        address _klimaSwapHolder,
        uint256 _klimaRetirementFee
    ) VaultAuthorization(authorizer) AssetHelpers(weth) TemporarilyPausable(pauseWindowDuration, bufferPeriodDuration) {
        // solhint-disable-previous-line no-empty-blocks
        klimaRetirementFee = _klimaRetirementFee;
        klimaSwapRetirementHolder = _klimaSwapHolder;
    ...
    function swapGreen(
        ...
        //execute swap first 
        uint256 amountCalc = super.swap(singleSwap,funds,limit, deadline);
        //send to KlimaSwap Retriement Holder contract  
        // Send native asset to retirementHolder address
        (bool sent, bytes memory data) = klimaSwapRetirementHolder.call{value: klimaRetirementFee}("");
