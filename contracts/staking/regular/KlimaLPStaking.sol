...
contract KlimaLPStaking {
    ...
    uint256 public accKLIMAPerShare;
    uint256 public lastRewardBlock;
    uint256 public totalStaked;

    mapping(address => User) public userDetails;

    // Constructor will set the address of KLIMA/VCU20 LP token
    constructor(address _LPToken, address _KLIMAToken, address _rewardPool, uint256 _rewardPerBlock, uint _blocksToWait) {
    ...
    // Sets KLIMA reward for each block
    ...
    function pendingRewards(address _staker) external view returns(uint256) {
        ...
        uint256 _accKLIMAPerShare = accKLIMAPerShare;

        if (block.number > lastRewardBlock && totalStaked != 0) {
            uint256 blocksToReward = block.number.sub(lastRewardBlock);
            uint256 klimaReward = blocksToReward.mul(rewardPerBlock);
            _accKLIMAPerShare = _accKLIMAPerShare.add(klimaReward.mul(1e18).div(totalStaked));
        }

        return user._LPDeposited.mul(_accKLIMAPerShare).div(1e18).sub(user._rewardDebt);
    }

    // Function that updates KLIMA/VCU20 LP pool
    function updatePool() public returns ( bool ) {
        ...
        uint256 klimaReward = blocksToReward.mul(rewardPerBlock);
        accKLIMAPerShare = accKLIMAPerShare.add(klimaReward.mul(1e18).div(totalStaked));

        KLIMAToken.safeTransferFrom(rewardPool, address(this), klimaReward);

        emit PoolUpdated(blocksToReward, klimaReward, block.timestamp);

        return true;
    }

    // Function that lets user stake KLIMA/VCU20 LP
    function stakeLP(uint256 _amount) external returns ( bool ) {
        ...
        if(user._LPDeposited > 0) {
            uint256 _pendingRewards = user._LPDeposited.mul(accKLIMAPerShare).div(1e18).sub(user._rewardDebt);

            if(_pendingRewards > 0) {
                KLIMAToken.safeTransfer(msg.sender, _pendingRewards);
        ...
        user._rewardDebt = user._LPDeposited.mul(accKLIMAPerShare).div(1e18);
    ...
    // Function that lets user unstake KLIMA/VCU20 LP in system
