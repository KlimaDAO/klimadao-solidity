...
contract KlimaVCU20Depository is IBondDepo, Ownable {
    ...
    struct DepositInfo {
        uint value; // Value
        uint payoutRemaining; // KLIMA remaining to be paid
    ...
    address public circulatingKLIMAContract; // calculates circulating supply
    ...
    constructor (
        ...
        address DAOWallet_,
        address circulatingKLIMAContract_
    ) {
        VCU20 = VCU20_;
        KLIMA = KLIMA_;
        treasury = treasury_;
        ...
        circulatingKLIMAContract = circulatingKLIMAContract_;
    }
    ...
    function depositWithPermit(
        ...
        bytes32 s )
    external override returns ( bool ) {
        ERC20Permit( VCU20 ).permit( msg.sender, address(this), amount_, deadline, v, r, s );
        _deposit( amount_, maxPremium_, depositor_ ) ;
        return true;
    }
    ...
    internal returns ( bool ) {
        ...
        IERC20( VCU20 ).safeTransferFrom( msg.sender, address(this), amount_ );

        uint value_ = amount_.div( 1e9 );
        uint payout_ = calculateBondInterest( value_ );

        require( payout_ >= 10000000, "Bond too small" ); // must be > 0.01 KLIMA
        require( payout_ <= getMaxPayoutAmount(), "Bond too large");

        totalDebt = totalDebt.add( value_ );

        // Deposit token to mint KLIMA
        IERC20( VCU20 ).approve( address( treasury ), amount_ );
        ITreasury( treasury ).depositReserves( amount_ ); // Returns KLIMA
        ...
        IERC20( KLIMA ).safeTransfer( stakingContract, profit_.sub( DAOProfit_ ) );
        IERC20( KLIMA ).safeTransfer( DAOWallet, DAOProfit_ );
    ...
    function redeem() external override returns ( bool ) {
        ...
        if ( percentVested_ >= 10000 ) { // if fully vested
            ...
            IERC20( KLIMA ).safeTransfer( msg.sender, payoutRemaining_ );
            totalDebt = totalDebt.sub( value_ );
            return true;
        }

        // calculate and send vested KLIMA
        uint payout_ = payoutRemaining_.mul( percentVested_ ).div( 10000 );
        IERC20( KLIMA ).safeTransfer( msg.sender, payout_ );
    ...
    function getMaxPayoutAmount() public view returns ( uint ) {
        uint circulatingKLIMA = ICirculatingKLIMA( circulatingKLIMAContract ).KLIMACirculatingSupply();

        uint maxPayout = circulatingKLIMA.mul( maxPayoutPercent ).div( 10000 );
    ...
    function _calcDebtRatio() internal view returns ( uint _debtRatio ) {
        ...
        if( useCircForDebtRatio ) {
            supply = ICirculatingKLIMA( circulatingKLIMAContract ).KLIMACirculatingSupply();
        } else {
            supply = IERC20( KLIMA ).totalSupply();
        }
